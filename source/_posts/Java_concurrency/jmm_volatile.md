...
监视器锁的 happens-before 规则保证释放监视器和获取监视器的两个线程之间的内存可见性，这意味着对一个volatile变量的读，总是能看到（任意线程）对这个volatile变量最后的写入。
...
**从`JSR-133`开始，volatile变量的写-读可以实现线程之间的通信。**
...
假设线程A执行writer()方法之后，线程B执行reader()方法。根据 happens before 规则，这个过程建立的happens before 关系可以分为两类：
...
![volatile-happens-before](jmm_volatile/volatile-happens-before.png)

> 在上图中，每一个箭头链接的两个节点，代表了一个happens before 关系。黑色箭头表示程序顺序规则；橙色箭头表示volatile规则；蓝色箭头表示组合这些规则后提供的happens before保证。

**`NOTE`这里A线程写一个volatile变量后，B线程读同一个volatile变量。A线程在写volatile变量之前所有可见的共享变量，在B线程读同一个volatile变量后，将立即变得对B线程可见。**
...
![volatile-write](jmm_volatile/volatile-write.png)
...
![volatile-write](jmm_volatile/volatile-read.png)
...
下面，让我们来看看**JMM如何实现volatile写/读的内存语义**。
...
![volatile-barrier](jmm_volatile/volatile-write-barrier.png)
...
![volatile-read-barrier](jmm_volatile/volatile-read-barrier.png)
...
![volatile-barrier](jmm_volatile/volatile-barrier.png)
...
![x86-barrier](jmm_volatile/x86-barrier.png)
...
![before-jsr-133](jmm_volatile/before-jsr-133.png)
...
整理自 [深入理解Java内存模型（四）——volatile](http://www.infoq.com/cn/articles/java-memory-model-4)