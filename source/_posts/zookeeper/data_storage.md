...
2. **数据同步初始化**。首先从Zookeeper内存数据库中提取出事务请求对应的提议缓存队列proposals，同时完成**peerLastZxid**(该Learner最后处理的ZXID)、**minCommittedLog**(Leader提议缓存队列commitedLog中最小的ZXID)、**maxCommittedLog**(Leader提议缓存队列commitedLog中的最大ZXID)三个ZXID值的初始化。
