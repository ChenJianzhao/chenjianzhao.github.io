...
- 使用 Redis 的 `List` 实现简单的先进先出队列
- 使用 Redis 的 有序集合 实现 延迟任务队列
...
把所有需要在未来执行的任务都添加到有序集合里面，并将任务的执行时间设置为分值，另外再使用一个进程来查找有序集合里面是否存在可以立即被执行的任务，如果有的话，就从有序集合里面移除那个任务，并将他添加到适当的任务队列里面。
